...
const MarkTeamAttendance = () => {
  ...
  // 1. Fetch team members (getEmployeesAPI intrinsically filters by managerId based on token)
  useEffect(() => {
  ...
  // 2. Fetch specific employee attendance when selected
  ...
  // Run fetch when selected employee changes
  useEffect(() => {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white/10 backdrop-blur-xl border border-white/20 p-6 rounded-2xl text-white">
        <h2 className="text-xl font-bold flex items-center gap-2 mb-4">
          <Users size={24} /> Mark Team Attendance
        </h2>
        <p className="text-white/60 text-sm mb-4">
          Select a team member below to view or mark their attendance for today.
        </p>
        ...
        <div className="max-w-md">
          <label className="text-sm text-white/60 block mb-2">Select Team Member</label>
          <select
            value={selectedEmpId}
          ...
          >
            <option value="" className="text-black">-- Choose an Employee --</option>
            {team.map(emp => (
